"""10k validation of new best config: Muon-VS + out_proj.

Compares against base Muon-VS (no out_proj) and Adam at 10k steps, 3 seeds.
"""
import sys
import json
import time
...
def run_one(name, argv_args):
    ...
    result_path = RESULTS_DIR / f"best10k_{name}.json"
...
def main():
    ...
    conditions = {
        # New best: Muon-VS + out_proj
        "vs_outproj": common + [
            "--optimizer", "muon", "--muon_variant", "vs",
            "--muon_lr", "0.02", "--adam_lr", "3e-4",
            "--muon_out_proj",
        ],
        # Previous best: Muon-VS without out_proj
        "vs_no_outproj": common + [
            "--optimizer", "muon", "--muon_variant", "vs",
            "--muon_lr", "0.02", "--adam_lr", "3e-4",
        ],
        # Adam baseline
    ...
    for seed in seeds:
        ...
        for config_name, base_args in conditions.items():
            ...
            ckpt = str(CKPT_DIR / f"best10k_{name}.pt")
    ...
    print(f"\n{'='*60}")
    print(f"BEST CONFIG 10K VALIDATION (total: {total_elapsed/60:.1f} min)")
    print(f"{'='*60}")
    ...
    print(f"  {'-'*18} {'-'*8} {'-'*8}  {'-'*35}")
    for c in ["vs_outproj", "vs_no_outproj", "adam"]:
        vals = configs.get(c, [])
        if len(vals) >= 2:
    ...
    # Paired analysis
    print(f"\n  Paired differences:")
    for c in ["vs_no_outproj", "adam"]:
        deltas = []
        for seed in seeds:
            best_r = next((r for r in results if r["config"] == "vs_outproj" and r["seed"] == seed), None)
            test_r = next((r for r in results if r["config"] == c and r["seed"] == seed), None)
            if best_r and test_r:
                deltas.append(test_r["val_loss"] - best_r["val_loss"])
        if len(deltas) >= 2:
            m = statistics.mean(deltas)
            s = statistics.stdev(deltas)
            t_stat = m / (s / len(deltas)**0.5) if s > 0 else float('inf')
            sig = "SIG" if abs(t_stat) > 2.92 else "n.s."
            print(f"    {c:<18s} vs vs_outproj: {m:+.4f} ± {s:.4f} (t={t_stat:.2f}, {sig})")

    summary_path = RESULTS_DIR / "best_10k_summary.json"
